...
public sealed class CustomerGroupsController(ISender sender) : ControllerBase
{
    ...
    [HttpPut("{id:guid}"), HasPermission(Permissions.Customer.Edit)] public Task<CustomerGroupDto> Update(Guid id, CustomerGroupInput input, CancellationToken token) => sender.Send(new UpdateCustomerGroup(id, input), token);
    [HttpDelete("{id:guid}"), HasPermission(Permissions.Customer.Delete)] public async Task<IActionResult> Delete(Guid id, CancellationToken token) { await sender.Send(new DeleteCustomerGroup(id), token); return NoContent(); }
}
