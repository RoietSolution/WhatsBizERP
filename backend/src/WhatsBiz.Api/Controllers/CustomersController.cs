...
public sealed class CustomersController(ISender sender, IEntityCodeService codes) : ControllerBase
{
    [HttpGet("code-preview"), HasPermission(Permissions.Customer.Create)] public Task<string> CodePreview(CancellationToken token) => codes.PreviewAsync(EntityCodeKind.Customer, token);
    ...
    [HttpPost, HasPermission(Permissions.Customer.Create)] public async Task<IActionResult> Create(CustomerInput input, CancellationToken token) { input = input with { CustomerCode = await codes.NextAsync(EntityCodeKind.Customer, token) }; var x = await sender.Send(new CreateCustomer(input), token); return CreatedAtAction(nameof(GetById), new { id = x.CustomerId }, x); }
    [HttpPut("{id:guid}"), HasPermission(Permissions.Customer.Edit)] public async Task<CustomerDto> Update(Guid id, CustomerInput input, CancellationToken token) { var existing = await sender.Send(new GetCustomer(id), token); return await sender.Send(new UpdateCustomer(id, input with { CustomerCode = existing.CustomerCode }), token); }
