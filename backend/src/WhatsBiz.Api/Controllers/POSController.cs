...
public sealed class POSController(ISender sender, IConfiguration configuration, ICurrentUserService currentUser) : ControllerBase
{
    ...
    [HttpPost("invoice"), HasPermission(Permissions.POS.Create)] public Task<PostedInvoiceDto> Invoice(POSInvoiceInput input, CancellationToken token)
    {
        EnforceCashierDiscount(input);
        return sender.Send(new PostInvoice(input), token);
    }
