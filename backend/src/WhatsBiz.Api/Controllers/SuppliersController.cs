...
public sealed class SuppliersController(ISender sender, IEntityCodeService codes) : ControllerBase
{
    [HttpGet("code-preview"), HasPermission(Permissions.Supplier.Create)] public Task<string> CodePreview(CancellationToken token) => codes.PreviewAsync(EntityCodeKind.Supplier, token);
    ...
    [HttpPost, HasPermission(Permissions.Supplier.Create)] public async Task<IActionResult> Create(SupplierInput input, CancellationToken token) { input = input with { SupplierCode = await codes.NextAsync(EntityCodeKind.Supplier, token) }; var x = await sender.Send(new CreateSupplierCommand(input), token); return CreatedAtAction(nameof(GetById), new { id = x.SupplierId }, x); }
    [HttpPut("{id:guid}"), HasPermission(Permissions.Supplier.Edit)] public async Task<SupplierDto> Update(Guid id, SupplierInput input, CancellationToken token) { var existing = await sender.Send(new GetSupplierQuery(id), token); return await sender.Send(new UpdateSupplierCommand(id, input with { SupplierCode = existing.SupplierCode }), token); }
