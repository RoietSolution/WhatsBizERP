...
public interface IAdminRepository
{
    ...
    Task<IReadOnlyCollection<AuditDto>> Audit(DateTimeOffset? f, DateTimeOffset? to, string? a, int take, CancellationToken t);
    Task<IReadOnlyCollection<LoginDto>> LoginHistory(DateTimeOffset? f, DateTimeOffset? to, int take, CancellationToken t);
    Task<IReadOnlyCollection<BackupDto>> Backups(CancellationToken t);
}

public interface IDatabaseMaintenanceService
{
    Task<BackupDto> Backup(string? user, CancellationToken token);
    Task<RestoreResultDto> Restore(RestoreInput input, string? user, CancellationToken token);
}
