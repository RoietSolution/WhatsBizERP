...
public sealed record UpdateCustomerGroup(Guid Id, CustomerGroupInput Input) : IRequest<CustomerGroupDto>;
public sealed record DeleteCustomerGroup(Guid Id) : IRequest;
...
public sealed class CustomerGroupHandlers(ICustomerGroupRepository repository, ICustomerRepository customers, ICurrentUserService user) : IRequestHandler<GetCustomerGroups, IReadOnlyCollection<CustomerGroupDto>>, IRequestHandler<CreateCustomerGroup, CustomerGroupDto>, IRequestHandler<UpdateCustomerGroup, CustomerGroupDto>, IRequestHandler<DeleteCustomerGroup>, IRequestHandler<GetCustomerGroupCustomers, PagedCustomers>
{
    ...
    public async Task<CustomerGroupDto> Handle(UpdateCustomerGroup request, CancellationToken cancellationToken) { var group = await repository.Find(request.Id, true, cancellationToken) ?? throw new WhatsBiz.Application.Common.Exceptions.EntityNotFoundException("Customer group not found."); var code=request.Input.GroupCode.Trim().ToUpperInvariant(); var name=request.Input.GroupName.Trim(); if (await repository.Exists(code, name, cancellationToken) && (!string.Equals(group.GroupCode, code, StringComparison.OrdinalIgnoreCase) || !string.Equals(group.GroupName, name, StringComparison.OrdinalIgnoreCase))) throw new WhatsBiz.Application.Common.Exceptions.BusinessRuleException("A customer group with this code or name already exists."); group.GroupCode = code; group.GroupName = name; group.IsActive = request.Input.IsActive; await repository.Save(cancellationToken); return Map(group); }
    public async Task Handle(DeleteCustomerGroup request, CancellationToken cancellationToken) { var group = await repository.Find(request.Id, true, cancellationToken) ?? throw new WhatsBiz.Application.Common.Exceptions.EntityNotFoundException("Customer group not found."); if (await repository.IsReferenced(request.Id, cancellationToken)) throw new WhatsBiz.Application.Common.Exceptions.BusinessRuleException("This customer group cannot be deleted because customers are assigned to it."); repository.Remove(group); await repository.Save(cancellationToken); }
