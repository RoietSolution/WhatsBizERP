...
public sealed record PrintingSettingsDto(string PaperSize, IReadOnlyCollection<string> SupportedPaperSizes);
public static class PaperSizes
{
    public const string Default = "80MM";
    public static readonly IReadOnlyCollection<string> Supported = ["58MM", "80MM", "A4"];
    public static string Normalize(string? value) => Supported.FirstOrDefault(x => string.Equals(x, value?.Trim(), StringComparison.OrdinalIgnoreCase)) ?? Default;
    public static bool IsSupported(string? value) => Supported.Any(x => string.Equals(x, value?.Trim(), StringComparison.OrdinalIgnoreCase));
}
...
public sealed record GetPrintingSettings : IRequest<PrintingSettingsDto>;
...
internal sealed class PrinterValidator : AbstractValidator<SavePrinter> { public PrinterValidator() { RuleFor(x => x.Input.PrinterName).NotEmpty().MaximumLength(200); RuleFor(x => x.Input.DisplayName).NotEmpty().MaximumLength(200); RuleFor(x => x.Input.PaperSize).Must(PaperSizes.IsSupported).WithMessage("Paper size must be 58MM, 80MM, or A4."); } }
internal sealed class DocumentValidator : AbstractValidator<GenerateDocument> { public DocumentValidator() { RuleFor(x => x.Input.DocumentType).NotEmpty(); RuleFor(x => x.Input.DocumentNumber).NotEmpty(); RuleFor(x => x.Input.BodyHtml).NotEmpty(); RuleFor(x => x.Input.PaperType).Must(PaperSizes.IsSupported).WithMessage("Paper size must be 58MM, 80MM, or A4."); RuleFor(x => x.Input.Output).Must(x => x is "pdf" or "html"); } }
...
internal sealed class GetPrintingSettingsHandler(IPrintRepository repository) : IRequestHandler<GetPrintingSettings, PrintingSettingsDto> { public async Task<PrintingSettingsDto> Handle(GetPrintingSettings request, CancellationToken token) => new(await repository.DefaultPaperSize(token), PaperSizes.Supported); }
