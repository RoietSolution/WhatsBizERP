...
public sealed class ProductCategoryInputValidator : AbstractValidator<ProductCategoryInput> { public ProductCategoryInputValidator() { RuleFor(x => x.CategoryCode).NotEmpty().MaximumLength(50); RuleFor(x => x.CategoryName).NotEmpty().MaximumLength(200); RuleFor(x => x.Description).MaximumLength(1000); RuleFor(x => x.DisplayOrder).GreaterThanOrEqualTo(0); } }
...
public sealed class BrandInputValidator : AbstractValidator<BrandInput> { public BrandInputValidator() { RuleFor(x => x.BrandCode).NotEmpty().MaximumLength(50); RuleFor(x => x.BrandName).NotEmpty().MaximumLength(200); RuleFor(x => x.Description).MaximumLength(1000); RuleFor(x => x.Logo).MaximumLength(500); } }
...
public sealed class UnitInputValidator : AbstractValidator<UnitOfMeasureInput> { public UnitInputValidator() { RuleFor(x => x.UnitCode).NotEmpty().MaximumLength(50); RuleFor(x => x.UnitName).NotEmpty().MaximumLength(200); RuleFor(x => x.ShortName).NotEmpty().MaximumLength(20); RuleFor(x => x.DecimalPlaces).InclusiveBetween((byte)0, (byte)6); } }
