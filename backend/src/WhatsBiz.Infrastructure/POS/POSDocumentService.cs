...
public sealed class POSDocumentService(IPrintingService printing, IConfiguration configuration) : IPOSDocumentService
{
    ...
    public string InvoiceHtml(SalesInvoice invoice, string paper, POSInvoicePrintContext context)
    {
        ...
        foreach (var item in invoice.Items)
            body.Append($"<tr><td>{Encode(item.Product.ProductName)}</td><td data-label=\"Qty\">{Quantity(item.Quantity)}</td><td data-label=\"Rate\">{Money(item.UnitPrice)}</td><td data-label=\"GST%\">{item.TaxPercentage:0.##}</td><td data-label=\"GST\">{Money(item.TaxAmount)}</td><td data-label=\"Amount\">{Money(item.LineTotal)}</td></tr>");
        body.Append($"""
              </tbody></table>
              <hr class="separator" />
