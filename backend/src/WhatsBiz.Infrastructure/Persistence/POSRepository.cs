...
public sealed class POSRepository(ApplicationDbContext db, ICurrentUserService currentUser) : IPOSRepository
{
    ...
    private IQueryable<SalesInvoice> TenantInvoices => db.SalesInvoices.Where(x => EF.Property<Guid?>(x, "TenantId") == Tenant);
    ...
    public async Task<IReadOnlyCollection<POSProductLookup>> Products(string? search, string? barcode, Guid? warehouseId, Guid? categoryId, Guid? brandId, int size, CancellationToken token)
    {
        ...
        var balances = await db.InventoryBalances.AsNoTracking()
            .Where(x => EF.Property<Guid?>(x, "TenantId") == Tenant && ids.Contains(x.ProductId) && x.WarehouseId == warehouseId && x.ZoneId == null && x.BinId == null && x.BatchNo == null && x.SerialNo == null)
    ...
    public Task<SalesInvoice?> Invoice(Guid id, CancellationToken token) => TenantInvoices.AsNoTracking().Include(x=>x.Customer).Include(x=>x.Warehouse).Include(x=>x.Items).ThenInclude(x=>x.Product).Include(x=>x.Payments).ThenInclude(x=>x.PaymentMethod).Include(x=>x.Taxes).Include(x=>x.Discounts).SingleOrDefaultAsync(x=>x.InvoiceId==id,token);
    public Task<SalesInvoice?> InvoiceForTenant(Guid id, Guid tenantId, CancellationToken token) => db.SalesInvoices.AsNoTracking().Where(x => EF.Property<Guid?>(x, "TenantId") == tenantId).Include(x=>x.Customer).Include(x=>x.Warehouse).Include(x=>x.Items).ThenInclude(x=>x.Product).ThenInclude(x=>x.Unit).Include(x=>x.Payments).ThenInclude(x=>x.PaymentMethod).Include(x=>x.Taxes).Include(x=>x.Discounts).SingleOrDefaultAsync(x=>x.InvoiceId==id,token);
    public async Task<(IReadOnlyCollection<SalesInvoice>,int)> Invoices(string? search,string? status,DateTimeOffset? from,DateTimeOffset? toDate,int page,int size,CancellationToken token) { IQueryable<SalesInvoice> q=TenantInvoices.AsNoTracking().Include(x=>x.Customer); if(!string.IsNullOrWhiteSpace(search))q=q.Where(x=>x.InvoiceNumber.Contains(search)||(x.Customer!=null&&x.Customer.CustomerName.Contains(search))); if(!string.IsNullOrWhiteSpace(status))q=q.Where(x=>x.Status==status); if(from.HasValue)q=q.Where(x=>x.InvoiceDate>=from); if(toDate.HasValue)q=q.Where(x=>x.InvoiceDate<=toDate); var count=await q.CountAsync(token); return (await q.OrderByDescending(x=>x.InvoiceDate).Skip((Math.Max(page,1)-1)*Math.Clamp(size,1,200)).Take(Math.Clamp(size,1,200)).ToArrayAsync(token),count); }
