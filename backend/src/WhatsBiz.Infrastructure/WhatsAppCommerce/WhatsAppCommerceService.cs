    ...
    ILoyaltyService? loyalty = null) : IWhatsAppCommerceService
{
    ...
    private Guid AuthenticatedTenant(Guid requested)
        => ResolveAuthenticatedTenant(currentUser, requested);

    internal static Guid ResolveAuthenticatedTenant(ICurrentUserService currentUser, Guid requested)
    {
    ...
    public async Task<WhatsAppCommerceSetup> GetSetupAsync(Guid tenantId, Guid? warehouseId, CancellationToken token)
    {
        ...
        await using (var command = new SqlCommand("SELECT WarehouseId,WarehouseCode,WarehouseName FROM inventory.Warehouses WHERE TenantId=@tenant AND IsActive=1 AND IsDeleted=0 ORDER BY IsDefault DESC,WarehouseName;", connection))
        { command.Parameters.AddWithValue("@tenant", tenantId);
        ...
        if (warehouseId.HasValue && warehouses.All(x => x.WarehouseId != warehouseId.Value)) throw new EntityNotFoundException("Warehouse was not found.");
    ...
    public async Task<WhatsAppCommerceCart> CalculateCartAsync(Guid tenantId, Guid warehouseId, IReadOnlyCollection<WhatsAppCommerceCartItem> items, CancellationToken token)
    {
        ...
        await RequireWarehouse(connection, tenantId, warehouseId, token);
    ...
    public async Task<WhatsAppCommerceReadiness> GetReadinessAsync(Guid tenantId, CancellationToken token)
    {
        ...
        string mode;
        await using (var modeCommand = new SqlCommand("SELECT TOP(1) ProviderMode FROM integration.WhatsAppConfigurations WHERE TenantId=@tenant AND IsEnabled=1;", connection))
        { modeCommand.Parameters.AddWithValue("@tenant", tenantId); mode = (await modeCommand.ExecuteScalarAsync(token) as string)?.ToUpperInvariant() ?? "NOT_CONFIGURED"; }
        var providerReady = mode switch
        {
        ...
        var checks = new List<WhatsAppCommerceReadinessCheck>
        {
            ...
            new("productsAvailable", "WhatsApp products available", await Exists("SELECT COUNT(1) FROM master.Products WHERE TenantId=@tenant AND IsWhatsAppVisible=1 AND IsActive=1 AND IsDeleted=0;"), "/products", "Mark at least one active product as available on WhatsApp Ecommerce."),
            new("stockAvailable", "WhatsApp product stock available", await Exists("SELECT COUNT(1) FROM inventory.InventoryBalances b JOIN master.Products p ON p.ProductId=b.ProductId AND p.TenantId=b.TenantId JOIN inventory.Warehouses w ON w.WarehouseId=b.WarehouseId AND w.TenantId=b.TenantId WHERE p.TenantId=@tenant AND b.TenantId=@tenant AND w.TenantId=@tenant AND p.IsWhatsAppVisible=1 AND p.IsActive=1 AND p.IsDeleted=0 AND w.IsActive=1 AND w.IsDeleted=0 AND b.QuantityAvailable>0;"), "/inventory", "Receive or adjust stock for a WhatsApp-visible product.")
        };
        return new(mode, checks.All(x => x.Ready), checks);
    }
    ...
    public async Task<IReadOnlyCollection<WhatsAppCommerceOrderSummary>> GetDeliveryOrdersAsync(Guid tenantId, DateTimeOffset? fromDate, DateTimeOffset? toDate, string? deliveryStatus, string? trackingNumber, CancellationToken token)
    {
        ...
        await using var connection = new SqlConnection(ConnectionString); await connection.OpenAsync(token);
        var rows = new List<WhatsAppCommerceOrderSummary>();
        await using var command = new SqlCommand("SELECT TOP(200)i.InvoiceId,i.InvoiceNumber,i.InvoiceDate,i.GrandTotal,i.Status,w.SourceChannel,w.ProviderMode,w.DeliveryStatus,w.CourierName,w.TrackingNumber,w.DispatchedOn,w.DeliveredOn,c.CustomerName,c.Mobile,w.DeliveryAddress,w.FulfillmentMethod,w.PaymentType FROM integration.WhatsAppCommerceOrders w JOIN sales.SalesInvoices i ON i.InvoiceId=w.InvoiceId AND i.TenantId=w.TenantId LEFT JOIN sales.Customers c ON c.CustomerId=i.CustomerId AND c.TenantId=w.TenantId WHERE w.TenantId=@tenant AND (@from IS NULL OR i.InvoiceDate>=@from) AND (@to IS NULL OR i.InvoiceDate<DATEADD(day,1,@to)) AND (@deliveryStatus IS NULL OR w.DeliveryStatus=@deliveryStatus) AND (@trackingNumber IS NULL OR w.TrackingNumber LIKE '%' + @trackingNumber + '%') ORDER BY i.InvoiceDate DESC;", connection);
    ...
    public async Task<WhatsAppCommerceOrderDetails> GetOrderAsync(Guid tenantId, Guid customerId, Guid orderId, CancellationToken token)
    {
        ...
        await using (var command = new SqlCommand("SELECT x.ProductId,p.ProductCode,p.ProductName,p.ImageUrl,x.Quantity,x.UnitPrice,x.TaxPercentage,x.TaxAmount,x.LineTotal,ISNULL(SUM(b.QuantityAvailable),0),c.CategoryName,u.UnitName FROM sales.SalesInvoiceItems x JOIN sales.SalesInvoices i ON i.InvoiceId=x.InvoiceId AND i.TenantId=@tenant JOIN master.Products p ON p.ProductId=x.ProductId AND p.TenantId=@tenant JOIN master.ProductCategories c ON c.ProductCategoryId=p.CategoryId JOIN master.UnitsOfMeasure u ON u.UnitId=p.UnitId LEFT JOIN inventory.InventoryBalances b ON b.ProductId=x.ProductId AND b.WarehouseId=i.WarehouseId AND b.TenantId=@tenant WHERE x.InvoiceId=@invoice GROUP BY x.ProductId,p.ProductCode,p.ProductName,p.ImageUrl,x.Quantity,x.UnitPrice,x.TaxPercentage,x.TaxAmount,x.LineTotal,c.CategoryName,u.UnitName;", connection))
        { command.Parameters.AddWithValue("@invoice", orderId); command.Parameters.AddWithValue("@tenant", tenantId); await using var reader = await command.ExecuteReaderAsync(token); while (await reader.ReadAsync(token)) items.Add(new(reader.GetGuid(0),reader.GetString(1),reader.GetString(2),reader.IsDBNull(3)?null:reader.GetString(3),reader.GetDecimal(4),reader.GetDecimal(5),reader.GetDecimal(6),reader.GetDecimal(7),reader.GetDecimal(8),reader.GetDecimal(9),reader.GetString(10),reader.GetString(11))); }
    ...
    public async Task<IReadOnlyCollection<WhatsAppCommerceMessage>> GetStatusNotificationsAsync(Guid tenantId, Guid customerId, CancellationToken token)
    {
        ...
        {
            await using var update = new SqlCommand("UPDATE integration.WhatsAppCommerceOrders SET LastNotifiedErpStatus=@status,LastNotifiedOn=SYSUTCDATETIME() WHERE WhatsAppCommerceOrderId=@id AND TenantId=@tenant AND ISNULL(LastNotifiedErpStatus,'')<>@status;", connection);
            update.Parameters.AddWithValue("@status",change.Status);update.Parameters.AddWithValue("@id",change.LinkId);update.Parameters.AddWithValue("@tenant",tenantId);
            if (await update.ExecuteNonQueryAsync(token) != 1) continue;
            if (mode == "MOCK")
            {
    ...
    private static async Task<IReadOnlyCollection<WhatsAppCommerceProduct>> Products(SqlConnection connection, Guid warehouseId, Guid tenantId, CancellationToken token)
    {
        ...
        await using var command = new SqlCommand("""
            SELECT p.ProductId,p.ProductCode,p.Barcode,p.ProductName,p.ShortDescription,p.ImageUrl,p.SellingPrice,p.MRP,p.GSTPercentage,
                   ISNULL(SUM(b.QuantityAvailable),0) AvailableQuantity,p.CategoryId,c.CategoryName,br.BrandName,u.UnitName
            FROM master.Products p JOIN master.ProductCategories c ON c.ProductCategoryId=p.CategoryId
            JOIN master.Brands br ON br.BrandId=p.BrandId
            JOIN master.UnitsOfMeasure u ON u.UnitId=p.UnitId
            JOIN inventory.Warehouses w ON w.WarehouseId=@warehouse AND w.TenantId=@tenant AND w.IsActive=1 AND w.IsDeleted=0
            LEFT JOIN inventory.InventoryBalances b ON b.ProductId=p.ProductId AND b.WarehouseId=w.WarehouseId AND b.TenantId=@tenant
        ...
        await using (var reader = await command.ExecuteReaderAsync(token))
        {
            while (await reader.ReadAsync(token)) rows.Add(new(reader.GetGuid(0), reader.GetString(1), reader.IsDBNull(2) ? null : reader.GetString(2), reader.GetString(3), reader.IsDBNull(4) ? null : reader.GetString(4), reader.IsDBNull(5) ? null : reader.GetString(5), reader.GetDecimal(6), reader.GetDecimal(7), reader.GetDecimal(8), reader.GetDecimal(9), reader.GetGuid(10), reader.GetString(11), reader.GetString(12), reader.GetString(13), []));
        }
        foreach (var product in rows.ToArray())
        {
            await using var images = new SqlCommand("SELECT TOP(5) ProductImageId FROM master.ProductImages WHERE ProductId=@product AND TenantId=@tenant AND IsActive=1 AND IsDeleted=0 ORDER BY IsPrimary DESC,CreatedOn;", connection);
            images.Parameters.AddWithValue("@product", product.ProductId);
            images.Parameters.AddWithValue("@tenant", tenantId);
    ...
    { var rows = new List<SendProduct>(); string? title = null; await using var command = new SqlCommand(@"
        ...
        WHERE c.CollectionId=@collection AND c.TenantId=@tenant AND c.IsActive=1 AND c.IsDeleted=0
          ...
          AND p.IsWhatsAppVisible=1 AND p.IsActive=1 AND p.IsDeleted=0 AND p.SellingPrice>=0
          AND EXISTS (SELECT 1 FROM inventory.InventoryBalances b JOIN inventory.Warehouses w ON w.WarehouseId=b.WarehouseId AND w.TenantId=b.TenantId WHERE b.ProductId=p.ProductId AND b.TenantId=c.TenantId AND w.TenantId=c.TenantId AND b.QuantityAvailable>0 AND w.IsActive=1 AND w.IsDeleted=0)
    ...
    private static async Task RequireWarehouse(SqlConnection connection, Guid tenantId, Guid warehouseId, CancellationToken token)
    { await using var command = new SqlCommand("SELECT COUNT(1) FROM inventory.Warehouses WHERE WarehouseId=@warehouse AND TenantId=@tenant AND IsActive=1 AND IsDeleted=0;", connection); command.Parameters.AddWithValue("@warehouse", warehouseId); command.Parameters.AddWithValue("@tenant", tenantId); if (Convert.ToInt32(await command.ExecuteScalarAsync(token), System.Globalization.CultureInfo.InvariantCulture) != 1) throw new EntityNotFoundException("Warehouse was not found."); }
