    ...
    ICommercePaymentService? payments = null) : IWhatsAppInboundCommerceHandler
{
    ...
    private async Task<string> ExecuteAsync(Guid tenantId, string mobile, Conversation conversation, string input, string messageId, CancellationToken token)
    {
        ...
        if (conversation.OrderId is Guid orderId)
        {
            if (command.Equals("pay razorpay", StringComparison.OrdinalIgnoreCase) || command.Equals("pay online", StringComparison.OrdinalIgnoreCase))
                return await CreatePayment(tenantId, orderId, PaymentProviders.Razorpay, token);
            if (command.Equals("pay upi", StringComparison.OrdinalIgnoreCase) || command.Equals("upi", StringComparison.OrdinalIgnoreCase))
                return await CreatePayment(tenantId, orderId, PaymentProviders.DirectUpi, token);
            if (command.Equals("cod", StringComparison.OrdinalIgnoreCase) || command.Contains("cash on delivery", StringComparison.OrdinalIgnoreCase))
                return await CreatePayment(tenantId, orderId, PaymentProviders.Cod, token);
        }
    ...
    private async Task<string> Confirm(Guid tenantId, Conversation c, string messageId, CancellationToken token)
    {
        ...
        try
        {
            ...
            var choices = methods.Count == 0 ? "No payment method is currently enabled; please contact the retailer."
    ...
    private sealed record Conversation(Guid ConversationId,Guid? CustomerId,Guid WarehouseId,Guid? OrderId);
}
