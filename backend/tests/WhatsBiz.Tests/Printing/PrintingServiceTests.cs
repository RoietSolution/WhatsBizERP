...
public sealed class PrintingServiceTests
{
    ...
    [Theory][InlineData("58MM", "58mm")][InlineData("80MM", "80mm")][InlineData("A4", "A4 portrait")] public void DocumentProducesPrintableHtml(string paper, string expectedPageSize) { var result = service.Document(new("SALES_INVOICE", "INV-1", "GST Invoice", "<p>Total 100</p>", paper, "html")); result.ContentType.Should().Be("text/html"); Encoding.UTF8.GetString(result.Data).Should().Contain($"size:{expectedPageSize}"); }
    [Fact] public void DocumentCanOmitWrapperHeader() { var result = service.Document(new("SALES_INVOICE", "INV-1", "GST Invoice", "<main>Receipt body</main>", Output: "html", IncludeHeader: false)); var html = Encoding.UTF8.GetString(result.Data); html.Should().NotContain("<header class='document-head'>"); html.Should().NotContain("INV-1"); html.Should().Contain("Receipt body"); }
