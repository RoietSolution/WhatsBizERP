...
IF EXISTS(SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID(N'master.ProductImages') AND name=N'StorageProvider' AND is_nullable=1)
    EXEC(N'ALTER TABLE master.ProductImages ALTER COLUMN StorageProvider nvarchar(20) NOT NULL;');
IF EXISTS(SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID(N'master.ProductImages') AND name=N'CatalogSizeBytes' AND is_nullable=1)
    EXEC(N'ALTER TABLE master.ProductImages ALTER COLUMN CatalogSizeBytes bigint NOT NULL;');
IF EXISTS(SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID(N'master.ProductImages') AND name=N'ThumbnailSizeBytes' AND is_nullable=1)
