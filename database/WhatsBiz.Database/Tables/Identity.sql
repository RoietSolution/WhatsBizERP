...
CREATE TABLE [core].[Users] (
    [Id] UNIQUEIDENTIFIER NOT NULL PRIMARY KEY, [UserName] NVARCHAR(256) NULL, [NormalizedUserName] NVARCHAR(256) NULL, [Email] NVARCHAR(256) NULL, [NormalizedEmail] NVARCHAR(256) NULL, [EmailConfirmed] BIT NOT NULL, [PasswordHash] NVARCHAR(MAX) NULL, [SecurityStamp] NVARCHAR(MAX) NULL, [ConcurrencyStamp] NVARCHAR(MAX) NULL, [PhoneNumber] NVARCHAR(MAX) NULL, [PhoneNumberConfirmed] BIT NOT NULL, [TwoFactorEnabled] BIT NOT NULL, [LockoutEnd] DATETIMEOFFSET NULL, [LockoutEnabled] BIT NOT NULL, [AccessFailedCount] INT NOT NULL, [TenantId] UNIQUEIDENTIFIER NULL, [AccountType] NVARCHAR(30) NOT NULL CONSTRAINT [DF_Users_AccountType] DEFAULT N'RETAILER', [CreatedOn] DATETIMEOFFSET NOT NULL, [CreatedBy] NVARCHAR(256) NULL, [ModifiedOn] DATETIMEOFFSET NULL, [ModifiedBy] NVARCHAR(256) NULL, [IsActive] BIT NOT NULL, [IsDeleted] BIT NOT NULL, [MustChangePassword] BIT NOT NULL CONSTRAINT [DF_Users_MustChangePassword] DEFAULT 0, [RowVersion] ROWVERSION NOT NULL, CONSTRAINT [FK_Users_Tenants] FOREIGN KEY ([TenantId]) REFERENCES [core].[Tenants]([TenantId]), CONSTRAINT [CK_Users_AccountScope] CHECK (([AccountType]=N'APPLICATION_OWNER' AND [TenantId] IS NULL) OR ([AccountType]=N'RETAILER' AND [TenantId] IS NOT NULL)));
GO
