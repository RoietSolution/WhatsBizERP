...
CREATE TABLE [master].[Products] (
    ...
    [Weight] DECIMAL(18,4) NULL, [Length] DECIMAL(18,4) NULL, [Width] DECIMAL(18,4) NULL, [Height] DECIMAL(18,4) NULL, [ImageUrl] NVARCHAR(500) NULL, [IsBatchManaged] BIT NOT NULL, [IsSerialManaged] BIT NOT NULL, [IsWhatsAppVisible] BIT NOT NULL CONSTRAINT [DF_Products_IsWhatsAppVisible] DEFAULT (1),
    ...
    CONSTRAINT [FK_Products_Categories] FOREIGN KEY ([CategoryId]) REFERENCES [master].[ProductCategories]([ProductCategoryId]), CONSTRAINT [FK_Products_Brands] FOREIGN KEY ([BrandId]) REFERENCES [master].[Brands]([BrandId]), CONSTRAINT [FK_Products_Units] FOREIGN KEY ([UnitId]) REFERENCES [master].[UnitsOfMeasure]([UnitId]), CONSTRAINT [FK_Products_Tenants] FOREIGN KEY ([TenantId]) REFERENCES [core].[Tenants]([TenantId]), CONSTRAINT [CK_Products_BarcodeType] CHECK ([BarcodeType]=N'CUSTOM' OR [BarcodeType]=N'QR' OR [BarcodeType]=N'CODE39' OR [BarcodeType]=N'UPCE' OR [BarcodeType]=N'UPCA' OR [BarcodeType]=N'UPC' OR [BarcodeType]=N'EAN8' OR [BarcodeType]=N'EAN13' OR [BarcodeType]=N'CODE128'));
GO
...
CREATE TABLE [master].[ProductImages] (
    ...
    CONSTRAINT [CK_ProductImages_StorageProvider] CHECK([StorageProvider]=N'S3' OR [StorageProvider]=N'LOCAL' OR [StorageProvider]=N'DATABASE'),
