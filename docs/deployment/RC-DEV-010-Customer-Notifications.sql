/* RC-DEV-010 CUSTOMER NOTIFICATIONS - existing database deployment.
   Rerunnable, non-destructive, defaults all external delivery OFF. */
SET NOCOUNT ON;
SET XACT_ABORT ON;
IF DB_NAME()<>N'WhatsBizERP' THROW 51000,'Safety stop: current database must be WhatsBizERP.',1;
...
END;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'integration.CustomerNotifications') AND name=N'IX_CustomerNotifications_Work') CREATE INDEX IX_CustomerNotifications_Work ON integration.CustomerNotifications(Status,NextAttemptOn,CreatedOn) INCLUDE(Channel,Recipient,AttemptCount);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'integration.CustomerNotifications') AND name=N'IX_CustomerNotifications_History') CREATE INDEX IX_CustomerNotifications_History ON integration.CustomerNotifications(CreatedOn DESC) INCLUDE(CustomerId,DocumentId,Channel,Status,AttemptCount);
...
BEGIN
 ...
 SELECT @CompanyId,d.SettingKey,d.SettingValue,d.DataType,N'Customer Notifications' FROM @Defaults d WHERE NOT EXISTS(SELECT 1 FROM admin.ApplicationSettings s WHERE s.CompanyId=@CompanyId AND s.SettingKey=d.SettingKey);
END;
SELECT CASE WHEN OBJECT_ID(N'integration.CustomerNotifications',N'U') IS NOT NULL THEN N'PASS' ELSE N'FAIL' END Result,N'integration.CustomerNotifications' ObjectName;
