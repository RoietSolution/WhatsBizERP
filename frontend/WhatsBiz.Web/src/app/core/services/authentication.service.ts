...
export class AuthenticationService {
  ...
  forgotPassword(identifier: string): Observable<ForgotPasswordResponse> {
    return this.http.post<ForgotPasswordResponse>('/api/auth/forgot-password', { identifier });
  }
  resetPassword(userId: string, token: string, newPassword: string): Observable<void> {
    return this.http.post<void>('/api/auth/reset-password', { userId, token, newPassword });
  }
  changePassword(currentPassword: string, newPassword: string): Observable<void> {
    return this.http.post<void>('/api/auth/change-password', { currentPassword, newPassword });
  }
  updateProfile(email: string): Observable<CurrentUser> {
    return this.http.put<CurrentUser>('/api/auth/profile', { email }).pipe(
      tap((user) => {
        const updated = { ...this.currentUser.user(), ...user } as CurrentUser;
        this.currentUser.set(updated);
        this.storage.updateUser(updated);
      }),
    );
  }
  ...
  refresh(): Observable<AuthSession> {
    ...
    this.refreshRequest = this.http.post<AuthSession>('/api/auth/refresh', { refreshToken }).pipe(
