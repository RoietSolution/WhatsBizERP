...
export class PaymentSettingsComponent implements OnInit {
  ...
  ngOnInit(){this.features.tenants().subscribe({next:x=>{this.tenants.set(x);if(x[0])this.selectTenant(x[0].tenantId);},error:()=>this.error.set('Only the Application Owner can load payment settings.')});}
  selectTenant(tenantId:string){this.selectedTenantId.set(tenantId);this.load();}
  load(){const tenant=this.selectedTenantId();if(!tenant)return;this.loading.set(true);this.api.settings(tenant).subscribe({next:x=>{this.bind(x);this.loading.set(false);},error:()=>{this.error.set('Payment settings could not be loaded.');this.loading.set(false);}});}
