...
import { SupplierList } from './supplier.models';
@Component({
...
export class SupplierListComponent {
  ...
  readonly items = signal<SupplierList[]>([]);
  ...
  sortBy = 'supplierName';
  descending = false;
  readonly config: MasterPageConfig<SupplierList> = {
    title: 'Suppliers',
    singular: 'Supplier',
    description: 'Manage vendors, GST details, credit terms, and purchasing relationships.',
    icon: 'local_shipping',
    ...
    columns: [
      { field: 'supplierCode', headerName: 'Code' },
      { field: 'supplierName', headerName: 'Supplier name', minWidth: 220 },
  ...
  load() {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: () => this.snack.open('Unable to load suppliers.', 'Dismiss', { duration: 4000 }),
      });
  }
  ...
  sorted(e: MasterSortEvent) {
    this.sortBy = e.field || 'supplierName';
    this.descending = e.direction === 'desc';
    this.page = 1;
    this.load();
  }
  handle(e: MasterActionEvent<SupplierList>) {
    if (e.action === 'refresh') this.load();
    else if (e.action === 'import') this.file()?.nativeElement.click();
    else if (e.action === 'export')
      this.api.export().subscribe((b) => download(b, 'suppliers.xlsx'));
    else if (e.action === 'print') window.print();
    else if (e.action === 'edit' && e.row)
      this.router.navigate(['/suppliers', e.row.supplierId, 'edit']);
    else if (e.action === 'delete' && e.rows?.length) this.remove(e.rows);
    else
      this.snack.open(`${e.action} is ready for the selected supplier(s).`, undefined, {
        duration: 2500,
      });
  }
  remove(rows: SupplierList[]) {
    this.dialog
      .open(ConfirmDialogComponent, {
        data: { title: 'Delete suppliers', message: `Delete ${rows.length} selected supplier(s)?` },
      })
      .afterClosed()
      .subscribe((ok) => {
        if (ok)
          rows.forEach((x, i) =>
            this.api.delete(x.supplierId).subscribe({
              complete: () => {
  ...
  upload(e: Event) {
    ...
    this.api.import(f).subscribe((x) => {
      input.value = '';
      this.snack.open(`Imported ${x.importedCount} supplier(s).`, undefined, { duration: 3000 });
      this.load();
    });
  }
}
