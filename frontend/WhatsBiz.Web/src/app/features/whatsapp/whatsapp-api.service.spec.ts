...
describe('WhatsAppApiService contacts',()=>{
  ...
  it('uses the authenticated administration path for subscription diagnostics',()=>{api.subscriptionDiagnostics('tenant-guturgo').subscribe();const request=http.expectOne('/api/whatsapp/administration/tenants/tenant-guturgo/configuration/subscription-diagnostics');expect(request.request.method).toBe('GET');request.flush({requestSucceeded:true,wabaId:'***5444',subscribedApplicationIds:['***3554'],khataDhariAppMatches:true,subscribedFields:['messages'],messagesSubscribed:true});});
});
