...
export class WhatsAppCommerceDemoApiService {
  ...
  private root(targetTenantId?:string){return targetTenantId ? `/api/whatsapp-commerce/administration/tenants/${targetTenantId}/demo` : '/api/whatsapp-commerce/demo';}
  setup(warehouseId?:string,targetTenantId?:string){return this.http.get<DemoSetup>(`${this.root(targetTenantId)}/setup`,{params:warehouseId?new HttpParams().set('warehouseId',warehouseId):undefined});}
  readiness(targetTenantId?:string){return this.http.get<DemoReadiness>(`${this.root(targetTenantId)}/readiness`);}
  cart(warehouseId:string,items:{productId:string;quantity:number}[],targetTenantId?:string){return this.http.post<DemoCart>(`${this.root(targetTenantId)}/cart`,{warehouseId,items});}
  order(customerId:string,warehouseId:string,items:{productId:string;quantity:number}[],deliveryAddress:string,fulfillmentMethod:string,paymentType:string,redeemCoins=0,targetTenantId?:string){return this.http.post<DemoOrder>(`${this.root(targetTenantId)}/orders`,{customerId,warehouseId,items,deliveryAddress,fulfillmentMethod,paymentType,redeemCoins});}
  ...
  orders(customerId:string,targetTenantId?:string){return this.http.get<OrderSummary[]>(`${this.root(targetTenantId)}/orders`,{params:new HttpParams().set('customerId',customerId)});}
  orderDetails(orderId:string,customerId:string,targetTenantId?:string){return this.http.get<OrderDetails>(`${this.root(targetTenantId)}/orders/${orderId}`,{params:new HttpParams().set('customerId',customerId)});}
  updateDelivery(orderId:string,deliveryStatus:string,courierName?:string,trackingNumber?:string,targetTenantId?:string){return this.http.put<OrderSummary>(`${this.root(targetTenantId)}/orders/${orderId}/delivery`,{deliveryStatus,courierName: courierName||undefined,trackingNumber: trackingNumber||undefined});}
  ...
  notifications(customerId:string,targetTenantId?:string){return this.http.post<DemoMessage[]>(`${this.root(targetTenantId)}/status-notifications`,null,{params:new HttpParams().set('customerId',customerId)});}
  ...
  analytics(event:CommerceAnalyticsEvent,targetTenantId?:string){const url=targetTenantId?`/api/whatsapp-commerce/administration/tenants/${targetTenantId}/analytics`:'/api/whatsapp-commerce/analytics';return this.http.post<void>(url,event);}
}
