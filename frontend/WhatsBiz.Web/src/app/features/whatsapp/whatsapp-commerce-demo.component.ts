...
export class WhatsAppCommerceDemoComponent implements OnDestroy {
  ...
  readonly fulfillmentOption=signal<FulfillmentOption|null>(null); readonly checkoutPaymentType=signal<CheckoutPaymentType|null>(null);
  ...
  customerId=''; warehouseId=''; draftMessage=''; deliveryAddress=''; deliveryStatus='PENDING'; courierName=''; trackingNumber=''; private quantities=new Map<string,number>(); private imageUrls:string[]=[]; private imageLoad?:Subscription; private readonly intentEngine=new LocalCommerceIntentEngine();
  ...
  place(){if(!this.customerId||!this.warehouseId||!this.items().length||!this.deliveryAddress.trim()||!this.fulfillmentOption()||!this.checkoutPaymentType()){this.error.set('Enter the address and select fulfilment and payment before confirming the order.');return;}this.busy.set(true);this.error.set('');this.api.order(this.customerId,this.warehouseId,this.items(),this.deliveryAddress.trim(),this.fulfillmentOption()!,this.checkoutPaymentType()!).subscribe({next:x=>{this.order.set(x);this.messages.update(m=>[...m,...x.messages]);this.showCart.set(false);this.quantities.clear();this.cart.set(null);this.busy.set(false);},error:()=>{this.error.set('The order could not be created. Check the checkout details, customer, warehouse, and stock availability.');this.busy.set(false);}});}
