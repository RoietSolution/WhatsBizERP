...
export class WhatsAppCommerceDemoComponent implements OnDestroy {
  ...
  constructor(private readonly api:WhatsAppCommerceDemoApiService,private readonly features:FeatureService,currentUser:CurrentUserService){this.applicationOwner=currentUser.user()?.roles.includes('ApplicationOwner')===true;if(this.applicationOwner)this.features.tenants().subscribe({next:x=>this.tenants.set(x),error:()=>this.error.set('Retailers could not be loaded.')});else this.checkReadiness();}
  get contextEyebrow(){return this.applicationOwner?'Platform administration':'WhatsApp Ecommerce';}
  get contextDescription(){return this.applicationOwner?'Select a retailer explicitly to preview its ERP catalogue, pricing and stock.':'Preview your retailer ERP catalogue, pricing and stock.';}
  get showRetailerSelector(){return this.applicationOwner;}
  selectTenant(){this.setup.set(null);this.readiness.set(null);this.messages.set([]);this.cart.set(null);this.order.set(null);this.started.set(false);this.error.set('');if(this.tenantId)this.checkReadiness();}
  checkReadiness(){if(this.applicationOwner&&!this.tenantId)return;this.busy.set(true);this.api.readiness(this.targetTenant()).subscribe({next:x=>{this.readiness.set(x);this.busy.set(false);if(x.ready)this.load();},error:()=>{this.error.set('Demo readiness could not be checked.');this.busy.set(false);}});}
  load(warehouseId?:string){if(this.applicationOwner&&!this.tenantId)return;this.busy.set(true);this.error.set('');this.api.setup(warehouseId,this.targetTenant()).subscribe({next:x=>{this.loadImages(x);this.messages.set(x.messages);this.warehouseId=warehouseId??x.warehouses[0]?.warehouseId??'';this.customerId=x.customers[0]?.customerId??'';},error:()=>{this.error.set("We couldn't load the ERP catalogue. Please try again.");this.busy.set(false);}});}
  ...
  discountPercentage(product:DemoProduct){return product.mrp>product.sellingPrice?Math.round((product.mrp-product.sellingPrice)*100/product.mrp):0;}
  loadOrders(){if(!this.customerId)return;this.api.orders(this.customerId,this.targetTenant()).subscribe({next:x=>this.orders.set(x),error:()=>this.error.set('Orders could not be loaded.')});}
  openOrder(x:OrderSummary){this.deliveryStatus=x.deliveryStatus;this.courierName=x.courierName??'';this.trackingNumber=x.trackingNumber??'';this.api.orderDetails(x.orderId,this.customerId,this.targetTenant()).subscribe({next:d=>this.orderDetails.set(d),error:()=>this.error.set('Order details could not be loaded.')});}
  updateDelivery(){const detail=this.orderDetails();if(!detail)return;this.busy.set(true);this.api.updateDelivery(detail.order.orderId,this.deliveryStatus,this.courierName,this.trackingNumber,this.targetTenant()).subscribe({next:updated=>{this.orders.update(rows=>rows.map(row=>row.orderId===updated.orderId?updated:row));this.orderDetails.set({...detail,order:updated});this.busy.set(false);this.error.set('');},error:()=>{this.error.set('Delivery status could not be updated.');this.busy.set(false);}});}
  refreshStatuses(){if(!this.customerId)return;this.api.notifications(this.customerId,this.targetTenant()).subscribe({next:x=>{if(x.length)this.messages.update(m=>[...m,...x]);this.myOrders();},error:()=>this.error.set('Order statuses could not be refreshed.')});}
  place(){if(!this.customerId||!this.warehouseId||!this.items().length||!this.deliveryAddress.trim()||!this.fulfillmentOption()||!this.checkoutPaymentType()){this.error.set('Enter the address and select fulfilment and payment before confirming the order.');return;}this.busy.set(true);this.error.set('');this.api.order(this.customerId,this.warehouseId,this.items(),this.deliveryAddress.trim(),this.fulfillmentOption()!,this.checkoutPaymentType()!,Number(this.redeemCoins)||0,this.targetTenant()).subscribe({next:x=>{this.order.set(x);this.messages.update(m=>[...m,...x.messages]);this.showCart.set(false);this.quantities.clear();this.cart.set(null);this.redeemCoins=0;this.loadWallet();this.busy.set(false);},error:error=>{this.error.set(this.safeApiError(error,'The order could not be created. Please verify the checkout details and try again.'));this.busy.set(false);}});}
  ...
  private calculate(){if(!this.warehouseId)return;this.cartLoad?.unsubscribe();const items=this.items();if(!items.length){this.cart.set(null);this.error.set('');return;}this.cartLoad=this.api.cart(this.warehouseId,items,this.targetTenant()).subscribe({next:x=>{this.cart.set(this.withLoadedImages(x));this.error.set('');},error:error=>this.error.set(this.safeApiError(error,'Cart could not be recalculated from current ERP pricing and stock.'))});}
  ...
  private targetTenant(){return this.applicationOwner?this.tenantId||undefined:undefined;}
  ...
  private respondToMessageV2(text:string){
    ...
    if(!result.products.length){
      ...
      this.api.analytics({eventType:'PRODUCT_SEARCH_NO_MATCH',customerId:this.customerId,metadata:{language:criteria.language,confidence:criteria.confidence,category:criteria.category,maxPrice:criteria.maxPrice,resultCount:0}},this.targetTenant()).pipe(catchError(()=>of(void 0))).subscribe();
